...
function normalizeFacture(f) {
  return {
    ...
    vendeur:       f.vendeur?.vendorName || f.vendeur || '',
...
function validateCommande(cmd) {
  ...
  // vendeur = utilisateur connecté, pas de validation manuelle
...
export default function OrderForm({ navigate }) {
  ...
  const [clientIdx, setClientIdx] = useState(0);           // index courant dans la liste clients
  ...
  // ── Client courant et sa facture ──
  const currentClient   = apiClients[clientIdx] || apiClients[0];
  const clientCommandes = commandes.filter(c =>
    c.clientId === currentClient?.clientCode || c.clientId === String(currentClient?.id)
  );
  const cmd    = editing ? draft : (clientCommandes[0] || { ...nouvelleCommande(Date.now()), clientId: currentClient?.clientCode || '' });
  const totaux = calcTotaux(cmd);
  const status = editing ? 'Edit' : 'Parcours';

  // ── Navigation dans la liste des clients (Destinataire) ──
  const goFirst = () => { if (!editing) setClientIdx(0); };
  const goPrev  = () => { if (!editing) setClientIdx(i => Math.max(0, i - 1)); };
  const goNext  = () => { if (!editing) setClientIdx(i => Math.min(apiClients.length - 1, i + 1)); };
  const goLast  = () => { if (!editing) setClientIdx(apiClients.length - 1); };
  ...
  const startEdit = () => {
    setDraft(JSON.parse(JSON.stringify(cmd)));
    setEditing(true);
  };
  ...
  const saveEdits = () => {
    ...
    const facturePayload = {
      ...draft,
      client:  { id: apiClients.find(c => c.clientCode === draft.clientId)?.id || draft.clientId },
      vendeur: { vendorName: currentUser },
    ...
    apiCall
      .then(saved => {
        if (isExisting) {
          ...
          setCommandes(prev => [...prev, { ...draft, id: saved.id || draft.id }]);
        }
      })
      .catch(() => {
        // Fallback local
  ...
  const deleteCmd = () => {
    if (!editing && cmd.id && window.confirm('Supprimer cette commande ?')) {
      ...
      <div style={{ maxWidth: 716, margin: '0 auto', background: '#f0f0f0', border: '2px solid #888', borderRadius: 4, boxShadow: '3px 3px 8px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ padding: '8px 10px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 10, padding: '4px 6px', background: '#e8e8e8', border: '1px solid #ccc', borderRadius: 2 }}>
            ...
            <span style={{ marginLeft: 'auto', color: '#0000cc', fontSize: 12 }}>[{status}] {clientIdx + 1}/{apiClients.length}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', marginBottom: 6 }}>
            <div>
              ...
              <input style={{ ...S.input, width: 130, ...(errors.invoiceDate ? { border: '1px solid #c00', background: '#fff0f0' } : {}) }}
            ...
            <button style={{ ...S.btnNav, width: 30, marginBottom: errors.invoiceDate ? 16 : 0 }}>📅</button>
            <div style={{ marginLeft: 'auto' }}>
              <span style={S.label}>Fait par</span>
              <input style={{ ...S.inputRO, width: 160 }} readOnly value={currentUser} />
            </div>
          </div>

          {/* ── Grille Destinataire ── */}
          <div style={{ marginBottom: 4 }}>
            <span style={{ ...S.label, marginBottom: 3 }}>
              Destinataire
              {errors.clientId && <span style={{ color: '#c00', marginLeft: 8 }}>{errors.clientId}</span>}
            </span>
            <div style={{ border: errors.clientId ? '1px solid #c00' : '2px inset #999', background: '#fff', height: 110, overflowY: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ background: '#d4d0c8', position: 'sticky', top: 0 }}>
                    ...
                    <th style={{ padding: '2px 6px', textAlign: 'left', borderRight: '1px solid #aaa', fontWeight: 600 }}>Code</th>
                    <th style={{ padding: '2px 6px', textAlign: 'left', borderRight: '1px solid #aaa', fontWeight: 600 }}>Nom</th>
                    <th style={{ padding: '2px 6px', textAlign: 'left', fontWeight: 600 }}>Téléphone</th>
                  </tr>
                </thead>
                <tbody>
                  {apiClients.map((c, i) => {
                    const sel = c.clientCode === (cmd.clientId || currentClient?.clientCode);
                    return (
                      <tr key={c.id}
                        style={{ background: sel ? '#0a246a' : i % 2 === 0 ? '#fff' : '#f5f5f5', cursor: 'pointer' }}
                        onClick={() => {
                          const newIdx = apiClients.findIndex(cl => cl.id === c.id);
                          if (newIdx >= 0) setClientIdx(newIdx);
                          setField('clientId', c.clientCode);
                          setErrors(ev => ({ ...ev, clientId: undefined }));
                        }}>
                        <td style={{ padding: '2px 4px', textAlign: 'center', color: sel ? '#fff' : '#000', fontSize: 10, borderRight: '1px solid #eee' }}>{sel ? '▶' : ''}</td>
                        <td style={{ padding: '2px 6px', color: sel ? '#fff' : '#000', fontWeight: 600, borderRight: '1px solid #eee' }}>{c.clientCode}</td>
                        <td style={{ padding: '2px 6px', color: sel ? '#fff' : '#000', borderRight: '1px solid #eee' }}>{c.clientName}</td>
                        <td style={{ padding: '2px 6px', color: sel ? '#ccc' : '#555', fontSize: 11 }}>{c.phone || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div style={{ marginBottom: 4 }}>
            <input style={S.inputRO} readOnly value={currentClient?.address || ''} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 8 }}>
            <input style={S.inputRO} readOnly value={currentClient?.phone || ''} />
            <input style={S.inputRO} readOnly value={currentClient?.clientCode || ''} />
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, justifyContent: 'flex-end', marginRight: 20 }}>
                ...
                <input readOnly value={totaux.due.toFixed(2) + ' D.A'}
