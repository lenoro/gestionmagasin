...
export default function Articles({ navigate }) {
  ...
  const handlePartSave = ({ action, article, id }) => {
    if (action === 'update') {
      ArticleAPI.update(article.id, article).catch(() => {});
      setArticles(prev => prev.map(a => a.articleCode === article.articleCode ? article : a));
      setSelectedId(article.articleCode);
    } else if (action === 'add') {
      ArticleAPI.create(article).then(saved => {
        setArticles(prev => [...prev, saved]);
        setSelectedId(saved.articleCode);
      }).catch(() => {
      ...
      setArticles(prev => {
        ...
        setSelectedId(updated[0]?.articleCode || null);
        return updated;
      });
    }
  };
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 500 }}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...F.th, width: 70, textAlign: 'right' }}>Commandé</th>
                ...
                {displayed.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ ...F.td(sel), textAlign: 'right' }}>{a.onOrder ?? 0}</td>
                      <td style={{
                        ...F.td(sel),
                        textAlign: 'right',
                        fontWeight: (a.indisponible ?? 0) > 0 ? 700 : 400,
                        color: sel ? '#fff' : (a.indisponible ?? 0) > 0 ? '#c00' : '#000',
                        borderRight: 'none',
                      }}>
                        {a.indisponible ?? 0}
                      </td>
          ...
          {selectedId && (() => {
            ...
            return (
              <div style={{ marginBottom: 10, padding: '8px 12px', background: '#fff', border: '1px solid #ccc', borderRadius: 2, fontSize: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 20px' }}>
                ...
                <span><strong>Cost :</strong> {a.cost?.toFixed(2)} D.A</span>
                <span><strong>List Price :</strong> {a.price?.toFixed(2)} D.A</span>
