...
export default function Recherche({ navigate }) {
  ...
  const [showDefineQuery, setShowDefineQuery] = useState(false);
  ...
  /* ── Chargement commandes quand client change ou filtre date change ── */
  useEffect(() => {
    if (!selectedClient) return;
    const params = { clientId: selectedClient.id };
    if (queryActive && appliedFrom) params.from = appliedFrom;
    if (queryActive && appliedTo)   params.to   = appliedTo;
    const apiCall = (queryActive && (appliedFrom || appliedTo))
      ? FactureAPI.search(params)
      : FactureAPI.getByClient(selectedClient.id);
    apiCall
      .then(data => setCommandes(data))
      .catch(() => setCommandes(COMMANDES.filter(c => c.clientId === selectedClient.id)));
  }, [selectedClient, queryActive, appliedFrom, appliedTo]);
            ...
            return (
              <div style={{ marginBottom: 10, padding: '8px 10px', background: '#fff', border: '1px solid #ccc', borderRadius: 2, fontSize: 12 }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  ...
                  <tbody>
                    {(cmd.items || []).map(l => {
                      const articleCode = l.articleCode || l.article?.articleCode || '';
                      const articleName = l.articleName || l.article?.articleName || '';
                      const unitPrice   = l.unitPrice ?? l.lineTotal / (l.quantity || 1) ?? 0;
                      const remise      = l.remise ?? 0;
                      const ext = unitPrice * (l.quantity || 0) * (1 - remise / 100);
                      return (
                        <tr key={l.id}>
                          <td style={{ padding: '2px 6px', borderBottom: '1px solid #eee', fontWeight: 700 }}>{articleCode}</td>
                          <td style={{ padding: '2px 6px', borderBottom: '1px solid #eee' }}>{articleName}</td>
                          <td style={{ padding: '2px 6px', borderBottom: '1px solid #eee', textAlign: 'right' }}>{unitPrice.toFixed(2)} D.A</td>
                          <td style={{ padding: '2px 6px', borderBottom: '1px solid #eee', textAlign: 'center' }}>{l.quantity}</td>
                          <td style={{ padding: '2px 6px', borderBottom: '1px solid #eee', textAlign: 'right', color: remise > 0 ? '#c00' : '#000' }}>{remise.toFixed(2)}%</td>
                          <td style={{ padding: '2px 6px', borderBottom: '1px solid #eee', textAlign: 'right', fontWeight: 600 }}>{ext.toFixed(2)} D.A</td>
                        </tr>
                      );
                    })}
                  </tbody>
