...
export default function ServiceRefForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      serviceRefApi.findAll().then(list => {
        const s = list.find(x => x.id === Number(id))
        if (s) setLibelle(s.libelle)
      })
    }
  }, [id, isEdit])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      if (isEdit) await serviceRefApi.update(Number(id), { libelle: libelle.trim() })
      else await serviceRefApi.create({ libelle: libelle.trim() })
      navigate('/services-ref')
    } catch {
  ...
  return (
    <div className="max-w-md">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">{isEdit ? 'Modifier' : 'Nouveau'} service</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded shadow p-6 space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate('/services-ref')}
