...
import { bonSortieGSApi } from '../../api/stockApi'
import type { BonSortieGS } from '../../types/stock'
...
export default function BonSortieFiche() {
  ...
  const [bon, setBon] = useState<BonSortieGS | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')

  const load = () => bonSortieGSApi.findById(Number(id)).then(setBon).catch(() => setError('Bon introuvable')).finally(() => setLoading(false))
  useEffect(() => { load() }, [id])

  const handleValider = async () => {
    if (!confirm('Valider ce bon ? Le stock sera mis à jour.')) return
    await bonSortieGSApi.valider(Number(id))
    load()
  }

  const handleAnnuler = async () => {
    if (!confirm('Annuler ce bon ?')) return
    await bonSortieGSApi.annuler(Number(id))
    load()
  }
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Bon de sortie {bon.numero}</h1>
        <div className="flex gap-2">
          {statut === 'BROUILLON' && (
            <>
          ...
          <button onClick={() => navigate('/stock-sorties')}
      ...
      <div className="bg-white rounded shadow p-6 space-y-3 text-sm mb-4">
        <Row label="Date">{bon.dateSortie}</Row>
        <Row label="Employé">{bon.employe ? `${bon.employe.nom} ${bon.employe.prenom || ''}` : '—'}</Row>
        <Row label="Dépôt">{bon.depot?.libelle || '—'}</Row>
        <Row label="Agent magasin">{bon.agentMagasin || '—'}</Row>
        <Row label="Motif">{bon.motif || '—'}</Row>
      ...
      <div className="bg-white rounded shadow p-6">
        <h2 className="font-semibold text-gray-700 mb-3">Articles sortis</h2>
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 text-gray-600">
            <tr>
              <th className="px-3 py-2 text-left">Produit</th>
              <th className="px-3 py-2 text-right">Qté demandée</th>
              <th className="px-3 py-2 text-right">Qté servie</th>
          ...
          <tbody>
            {(bon.lignes ?? []).map((l, i) => (
              <tr key={i} className="border-t">
                ...
                <td className="px-3 py-2 text-right">{l.quantiteDemandee}</td>
                <td className="px-3 py-2 text-right">{l.quantiteServie ?? '—'}</td>
