...
export default function EditPart({ articleId, allArticles, onSave, onClose }) {
  const [idx, setIdx]       = useState(() => { const i = allArticles.findIndex(a => a.articleCode === articleId); return i >= 0 ? i : 0; });
  const [editing, setEditing] = useState(true);
  const [draft, setDraft]   = useState({ ...allArticles[Math.max(0, allArticles.findIndex(a => a.articleCode === articleId))] });
  ...
  const handleOk = () => {
    if (editing) {
      ...
      setErrors({});
      const exists = allArticles.find(a => a.articleCode === draft.articleCode);
    ...
    >
      <div style={{ width: 400, background: '#d4d0c8', border: '2px solid #888', borderRadius: 3, boxShadow: '4px 4px 10px rgba(0,0,0,0.4)', fontFamily: 'Tahoma, Arial, sans-serif', fontSize: 13 }}>
        ...
        <div style={{ padding: '10px 14px' }}>
          ...
          {/* Commandé */}
          <div style={S.row}>
            <span style={S.label}>Commandé</span>
            <input
              style={inputStyle('right')}
              type={editing ? 'number' : 'text'}
              min="0"
          ...
          {/* Indisponible */}
          <div style={S.row}>
            <span style={S.label}>Indisponible</span>
            <input
              style={{ ...inputStyle('right'), ...(art.indisponible > 0 ? { color: '#c00', fontWeight: 700 } : {}) }}
              type={editing ? 'number' : 'text'}
              min="0"
              value={art.indisponible ?? 0}
              onChange={e => setF('indisponible', parseInt(e.target.value) || 0)}
              readOnly={!editing}
            />
          </div>
          ...
          <div style={{ ...S.row, alignItems: 'flex-start', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
              ...
              <input
                ...
                type={editing ? 'number' : 'text'}
                min="0" step="0.01"
                value={editing ? (art.cost ?? 0) : ((art.cost ?? 0).toFixed(2) + ' D.A')}
          ...
          <div style={{ ...S.row, alignItems: 'flex-start', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
              ...
              <input
                ...
                type={editing ? 'number' : 'text'}
                min="0" step="0.01"
                value={editing ? (art.price ?? 0) : ((art.price ?? 0).toFixed(2) + ' D.A')}
