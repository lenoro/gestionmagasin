            ...
            {slice.map((cmd, i) => {
              ...
              return (
                <tr key={cmd.id} style={{ background: i % 2 === 0 ? '#fff' : '#f7f7f5' }}>
                  ...
                  <TD align="right" bold color={t.due > 0 ? '#c00' : '#060'}>{t.due.toFixed(2)} D.A</TD>
                </tr>
              );
            })}
          </tbody>
          {p === total - 1 && (
            <tfoot>
              <tr style={{ background: HDR_BG }}>
                ...
                <td style={{ padding: '5px 8px', textAlign: 'right', color: '#ffcccc', fontWeight: 700, fontSize: 12 }}>{grandDue.toFixed(2)} D.A</td>
              </tr>
            </tfoot>
          )}
        </table>
            ...
            {(cmd.items || []).map((l, i) => {
              ...
              return (
                <tr key={l.id} style={{ background: i % 2 === 0 ? '#fff' : '#f7f7f5' }}>
                  ...
                  <TD align="right">{unitPrice.toFixed(2)} D.A</TD>
                  <TD align="center">{l.quantity}</TD>
                  <TD align="right" bold>{ext.toFixed(2)} D.A</TD>
                </tr>
              );
            })}
          </tbody>
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 12, minWidth: 220 }}>
            <tr style={{ borderTop: '2px solid #0a246a' }}>
              ...
              <td style={{ padding: '5px 0', fontWeight: 700, fontSize: 14, textAlign: 'right', color: '#0a246a' }}>
                {(t.due ?? t.subtotal ?? 0).toFixed(2)} D.A
              </td>
            </tr>
          </table>
        </div>
