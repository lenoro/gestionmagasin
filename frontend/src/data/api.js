...
export const FactureAPI = {
  ...
  search:      (params)   => { const q = new URLSearchParams(Object.entries(params).filter(([,v]) => v != null && v !== '')); return fetch(`${BASE_URL}${API_PREFIX}/factures/search?${q}`, { headers: h() }).then(r => { if (!r.ok) throw new Error(r.status); return r.json(); }).then(d => Array.isArray(d) ? d : (d.content ?? [])); },
